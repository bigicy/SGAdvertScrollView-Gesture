...
#import "SGAdvertScrollGesture.h"
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    if (self.advertScrollViewStyle == SGAdvertScrollViewStyleMore) {
        ...
        SGAdvertScrollGesture *topTap = [[SGAdvertScrollGesture alloc]initWithTarget:self action:@selector(TapGestureAction:)];
        topTap.isTop = YES;
        topTap.indexPath = indexPath;
        [cell.topLabel addGestureRecognizer:topTap];
        ...
        SGAdvertScrollGesture *bottomTap = [[SGAdvertScrollGesture alloc]initWithTarget:self action:@selector(TapGestureAction:)];
        bottomTap.indexPath = indexPath;
        bottomTap.isTop = NO;
        [cell.bottomLabel addGestureRecognizer:bottomTap];
        if (self.titleFont) {
...
- (void)TapGestureAction:(SGAdvertScrollGesture *)tap
{
        if (tap.isTop) {
            [self.delegate advertScrollView:self didSelectedItemAtIndex:[NSIndexPath indexPathForRow:0 inSection:tap.indexPath.row]];
        }else{
            [self.delegate advertScrollView:self didSelectedItemAtIndex:[NSIndexPath indexPathForRow:1 inSection:tap.indexPath.row]];
        }
}


- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    if (self.advertScrollViewStyle == SGAdvertScrollViewStyleNormal) {
        if (self.delegate && [self.delegate respondsToSelector:@selector(advertScrollView:didSelectedItemAtIndex:)]) {
            [self.delegate advertScrollView:self didSelectedItemAtIndex:[NSIndexPath indexPathForRow:0 inSection:indexPath.row]];
        }
    }
}
