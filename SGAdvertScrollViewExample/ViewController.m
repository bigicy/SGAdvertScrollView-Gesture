...
- (void)viewDidLoad {
    ...
    _advertScrollViewBottom.delegate = self;
...
/// 代理方法
- (void)advertScrollView:(SGAdvertScrollView *)advertScrollView didSelectedItemAtIndex:(NSIndexPath *)indexPath {
    if (indexPath.row == 0) {
        NSLog(@"%@-----%ld",@"点击了第一个",indexPath.section);
    }else{
        NSLog(@"%@------%ld",@"点击了第二个",indexPath.section);
